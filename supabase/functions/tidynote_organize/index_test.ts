...
/** The stubbed OpenAI completion content: the model's raw output, classification
...
function makeDeps(options: StubOptions = {}) {
  ...
  const deps: Deps = {
    ...
    consumeRate: (key, window, limit) => {
      ...
      return options.rate
        ? options.rate(key, window, limit)
        : Promise.resolve(true);
    },
    consumeQuota: (userId, month, limit) => {
      calls.quota.push({ userId, month, limit });
      return options.quota
        ? options.quota(userId, month, limit)
        : Promise.resolve({ allowed: true, used: 1 });
    },
  };
...
function makeVoiceDeps(
  options: StubOptions & { whisper?: () => Promise<Response> } = {},
) {
  const whisper = options.whisper ?? (() => Promise.resolve(whisperSuccess()));
  const rest: NonNullable<StubOptions["fetchImpl"]> = options.fetchImpl ??
    (() => Promise.resolve(openAiSuccess()));
  return makeDeps({
    ...options,
    fetchImpl: (
      url,
      init,
    ) => (url.includes("/audio/transcriptions") ? whisper() : rest(url, init)),
  });
}
...
Deno.test("malformed appUserId is rejected before anything else runs", async () => {
  ...
  for (
    ...
    const response = await handleRequest(
      makeRequest({ text: "hello there", appUserId }),
      deps,
    );
    assertEquals(response.status, 400);
    assertEquals((await response.json()).error.code, "invalid_request");
...
Deno.test("empty and whitespace-only text are rejected", async () => {
  ...
  for (const text of ["", "   \n\t  "]) {
    const response = await handleRequest(
      makeRequest({ text, appUserId: VALID_USER }),
      deps,
    );
    assertEquals(response.status, 400);
    assertEquals((await response.json()).error.code, "invalid_request");
...
Deno.test("text over 60000 characters is too long", async () => {
  const { deps, calls } = makeDeps();
  const response = await handleRequest(
    makeRequest({ text: "a".repeat(60_001), appUserId: VALID_USER }),
    deps,
  );
...
Deno.test("text at exactly 60000 characters is accepted", async () => {
  const { deps } = makeDeps();
  const response = await handleRequest(
    makeRequest({ text: "a".repeat(60_000), appUserId: VALID_USER }),
    deps,
  );
...
Deno.test("per-user and per-IP limits use the right keys, windows and ceilings", async () => {
  const { deps, calls } = makeDeps();
  await handleRequest(
    makeRequest({ text: "hello there", appUserId: VALID_USER }, {
  ...
  assertEquals(calls.rate[0], {
    key: `u:${VALID_USER}`,
    window: "2026-08-07T14:03",
    limit: 6,
  });
...
Deno.test("exceeding the user limit returns 429 rate_limited and never reaches the model", async () => {
  const { deps, calls } = makeDeps({
    rate: (key) => Promise.resolve(!key.startsWith("u:")),
  });
  const response = await handleRequest(
    makeRequest({ text: "hello there", appUserId: VALID_USER }),
    deps,
...
Deno.test("exceeding the IP limit returns 429 rate_limited", async () => {
  const { deps } = makeDeps({
    rate: (key) => Promise.resolve(!key.startsWith("ip:")),
  });
  const response = await handleRequest(
    makeRequest({ text: "hello there", appUserId: VALID_USER }, {
      "x-forwarded-for": "203.0.113.9",
...
Deno.test("a broken rate limiter does not block the request", async () => {
  const { deps } = makeDeps({
    rate: () => Promise.reject(new Error("db down")),
  });
  const response = await handleRequest(
    makeRequest({ text: "hello there", appUserId: VALID_USER }),
    deps,
...
Deno.test("an exhausted quota returns 429 with the quota block", async () => {
  const { deps, calls } = makeDeps({
    quota: () => Promise.resolve({ allowed: false, used: 5 }),
  });
  const response = await handleRequest(
    makeRequest({ text: "hello there", appUserId: VALID_USER }),
    deps,
      ...
      Promise.resolve(
        url.includes("revenuecat")
          ? new Response(
            JSON.stringify({
              subscriber: {
                entitlements: { pro: { expires_date: "2020-01-01T00:00:00Z" } },
              },
            }),
            { status: 200 },
          )
...
Deno.test("a resolved entitlement is cached, a failed one is not", async () => {
  ...
  assertEquals(
    await resolvePlan(failingDeps, "tidy:other-user-id-value"),
    "free",
  );
  assertEquals(
    await resolvePlan(failingDeps, "tidy:other-user-id-value"),
...
Deno.test("a successful tidy returns note, quota and plan", async () => {
  _resetEntitlementCache();
  const { deps } = makeDeps({
    quota: () => Promise.resolve({ allowed: true, used: 3 }),
  });
  const response = await handleRequest(
    makeRequest({
...
Deno.test("note is sanitized before it leaves", async () => {
  ...
  const { deps } = makeDeps({
    fetchImpl: () => Promise.resolve(openAiSuccess(dirtyCompletion)),
  });
  const response = await handleRequest(
    makeRequest({ text: "call the dentist", appUserId: VALID_USER }),
    deps,
  );
  const body = await response.json();
  assertEquals(body.note.sections.length, 1);
  assertEquals(body.note.sections[0].items, [{
    text: "Buy milk",
    done: false,
  }]);
});
...
Deno.test('the upstream request carries the model and wraps the text with source="shared"', async () => {
  const { deps, calls } = makeDeps({
...
Deno.test("reasoning models are sent no temperature", async () => {
  const { deps, calls } = makeDeps({
    env: { OPENAI_API_KEY: "sk-test", TIDYNOTE_OPENAI_MODEL: "gpt-5-mini" },
  });
  await handleRequest(
    makeRequest({ text: "call the dentist", appUserId: VALID_USER }),
    deps,
...
Deno.test("the default model is used when the env var is unset", async () => {
  ...
  assertEquals(
    JSON.parse(String(calls.fetch[0].init?.body)).model,
    "gpt-5-mini",
  );
});
...
Deno.test("a model refusal becomes 502 upstream_error", async () => {
  const { deps } = makeDeps({
    fetchImpl: () =>
      Promise.resolve(
        new Response(
          JSON.stringify({ choices: [{ message: { refusal: "no" } }] }),
          { status: 200 },
        ),
      ),
  });
...
Deno.test("audio over 12 MB is refused before a quota is spent on it", async () => {
  const { deps, calls } = makeVoiceDeps();
  const response = await handleRequest(
    makeVoiceRequest({ audio: audioFile(13 * 1024 * 1024) }),
    deps,
  );
  assertEquals(response.status, 413);
...
Deno.test("a recording longer than the ceiling is refused before a quota is spent on it", async () => {
  const { deps, calls } = makeVoiceDeps();
  const response = await handleRequest(
    makeVoiceRequest({ durationSeconds: "400" }),
    deps,
  );
  assertEquals(response.status, 413);
  assertEquals((await response.json()).error.code, "audio_too_large");
...
Deno.test("a voice tidy charges once, then organizes what Whisper heard", async () => {
  _resetEntitlementCache();
  const { deps, calls } = makeVoiceDeps({
    quota: () => Promise.resolve({ allowed: true, used: 1 }),
  });
...
Deno.test('the voice path sends source="voice" and the text path sends source="shared"', async () => {
  const { deps: voiceDeps, calls: voiceCalls } = makeVoiceDeps();
  await handleRequest(makeVoiceRequest(), voiceDeps);
  const voiceSent = JSON.parse(String(voiceCalls.fetch[1].init?.body));
  assertStringIncludes(voiceSent.messages[1].content, 'source="voice"');

  const { deps: textDeps, calls: textCalls } = makeDeps();
  await handleRequest(
    makeRequest({ text: "call the dentist", appUserId: VALID_USER }),
    textDeps,
  );
  const textSent = JSON.parse(String(textCalls.fetch[0].init?.body));
  assertStringIncludes(textSent.messages[1].content, 'source="shared"');
});
...
Deno.test("the Whisper request carries the file, the model, plain text and the style prompt", async () => {
  ...
  assertStringIncludes(
    ...
    "normal punctuation and sentence casing",
  );
...
Deno.test("the Whisper model is swappable without a deploy", async () => {
  const { deps, calls } = makeVoiceDeps({
    env: { OPENAI_API_KEY: "sk-test", TIDYNOTE_WHISPER_MODEL: "whisper-next" },
  });
  await handleRequest(makeVoiceRequest(), deps);
...
Deno.test("a failed transcription is 502, and the quota it already spent stays spent", async () => {
  const { deps, calls } = makeVoiceDeps({
    whisper: () => Promise.resolve(new Response("boom", { status: 500 })),
  });
  const response = await handleRequest(makeVoiceRequest(), deps);
...
Deno.test("a transcript of nothing but whitespace is 422 empty_transcript", async () => {
  const { deps, calls } = makeVoiceDeps({
    whisper: () => Promise.resolve(whisperSuccess("   \n\t  ")),
  });
  const response = await handleRequest(makeVoiceRequest(), deps);
